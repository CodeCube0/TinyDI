...
- **Language banner**: only the user-triggered _dismiss_ is animated (fade + slight rise, then removed from layout). The _initial appearance_ is deliberately never animated — it's decided synchronously pre-paint (see `langBannerInitScript` in `templates/layout.mjs`) specifically so nothing shifts the layout on load; animating that reveal would reintroduce the exact bug that fix was written to prevent.
