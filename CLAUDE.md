...
- The task spec describes further phases not yet implemented: governance/CI/npm publishing + `ROADMAP.md` (`task/04_...`), and an internal blog + closing write-up (`task/05_...`). The package has never been published to npm yet — `README.md`/`README.it.md`/`docs-app` all document `npm install tinydi` as the intended future install path; until Phase 4 publishes it, consume the local build via `file:` (as `examples/` does) or `npm link`. `PRODUCT.md` and `DESIGN.md` at the repo root are the Impeccable-skill strategic/visual-design source of truth behind `docs-app/` (register: brand; Vite-like dark-first direction, one accent, self-hosted fonts) — read them before changing the site's visual language. `docs-app/src/nav.mjs`'s `SITE_URL` is a placeholder domain (`https://tinydi.dev`) used for `hreflang`/canonical/sitemap URLs until a real domain exists.
