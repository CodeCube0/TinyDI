...
container.registerInstance(ConfigToken, { apiUrl: 'https://example.com' });`,
  ...
  {
    type: 'p',
    html: 'The third, optional argument is a <a href="lifetimes.html"><code>ServiceLifetime</code></a> — it defaults to <code>Singleton</code>.',
  },
  ...
  { type: 'code', lang: 'ts', code: 'const userService = container.resolve(UserServiceToken);' },
  {
    ...
    kind: 'warning',
    title: 'Edge cases',
    html: 'Throws <code>ResolutionError</code> if the token was never registered, and <code>CircularDependencyError</code> if resolving it would require resolving itself again — see the <a href="api-reference.html#errors">API Reference</a> for the exact error shapes.',
  },
...
container.clear(); // removes every registration`,
  },
  {
    ...
    html: 'Registering an already-registered token throws <code>RegistrationError</code>, on purpose. Call <code>remove()</code> first — this is deliberately useful for swapping in a fake implementation between tests. See <a href="testing.html">Testing</a> for the full pattern.',
  },
];
